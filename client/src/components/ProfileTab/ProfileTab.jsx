      ...
      <div className={cx("container", "flex")}>
        <div className={cx("left-content")}>
          <div className={cx("user-profile-mini")}>
            ...
            <BasicButton content="Follow" />
          </div>
        </div>
        <div className={cx("right-content")}>
          <div className={cx("tab-list")}>
            <NavLink to={`/@${user?.username}?tab=project`} className={cx("tab-list__item", "active")}>
              <BsFillArchiveFill size={15} />
              Project
              <span className={cx("counter")}>40</span>
            </NavLink>
            <NavLink to={`/@${user?.username}?tab=star`} className={cx("tab-list__item")}>
              <BsStar size={15} />
              Star
              <span className={cx("counter")}>20</span>
            </NavLink>
            <NavLink to={`/@${user?.username}?tab=follower`} className={cx("tab-list__item")}>
              Followers
              <span className={cx("counter")}>1k</span>
            </NavLink>
            <NavLink to={`/@${user?.username}?tab=following`} className={cx("tab-list__item")}>
              Following
